(** RPC definitions for client-server communication — the wire protocol of
    ...
    Sessions replace explicit identity: no RPC below carries the {e
    sender's} {!Player_id.t}. The server remembers which player joined on
    which connection, so one client cannot drive another team's car. Only
    ...
    (four booleans, bare variants) or re-checked by the server ([name]
    below goes through {!Player_name.of_string}; interference targets are
    validated before they apply). Server-to-client data is trusted — the
    server built it. *)
...
module Car : sig
  (** Where one driver's car is and how it is moving — the per-driver motion
      state pushed in every snapshot. This will grow (laps, visual
      condition, ...) when the track model lands; keep any new field
      server-computed machine data, like these. *)
  type t =
    { position : Position.t
    ; velocity : Velocity.t
    }
...
module Game_snapshot : sig
  (** One complete statement of the world, pushed to every client every
      tick. There is no [track] field yet — the track model is being
      designed separately; it slots in here when it exists. *)
  type t =
...
(** Join as a named player on a team, as [Driver] or [Track_player].
    Returns the server-assigned {!Player_id.t} that identifies this player
    in every snapshot. Errors: invalid name, seat already taken, or this
...
(** THE feed: one complete {!Game_snapshot.t} per tick. Confirmation of any
    input is the next snapshot, never a response value — state flows one
    way, out of this pipe. *)
...
(** Driver uses a powerup they hold. Errors: not in inventory, not a
    driver, race not running. *)
...
(** Track player grants a powerup from their stock to their own driver, who
    can then {!use_powerup_rpc} it at the right moment. Errors: not in
    stock, not a track player. *)
