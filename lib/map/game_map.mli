(** The immutable track layout: surface and environment grids, checkpoint
    ...
    - checkpoint indexes are exactly [0 .. n-1] with [n >= 2], their cells
      on [Road];
    - every checkpoint is BFS-reachable from the previous one over
      non-solid cells even with EVERY gate closed and EVERY bridge
      collapsed (gaps impassable), so no combination of sabotage can make
      a lap impossible;
