...
(** A player's display name, as typed by a human when joining — so it is
    ...
    is unrepresentable. ([bin_io] is the one exception — a machine format for
    RPC transport of server-authored data. The name a client {e types}
    crosses the wire as a plain [string] in the join request and goes through
    {!of_string} on the server.) Names are for display only and are not
    guaranteed unique — key data structures by {!Player_id.t} instead. *)
