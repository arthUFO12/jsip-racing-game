(** RPC definitions for client-server communication — the wire protocol of
    ...
    below goes through {!Player_name.of_string}; sabotage targets are
...
module Sabotage : sig
  (** Everything a track player can invoke, unified for one RPC. The two
      ...
      effects are {!Interference.t} (domain vocabulary), map-targeted
      actions are {!Track_action.t} (validated against feature ids and
      phases by {!Map_state.apply_action}). This protocol type is where
      they meet — see the note in [lib/map/track_action.mli]. *)
  type t =
...
(** Join as a named player on a team, as [Driver] or [Track_player].
    Errors: invalid name, seat already taken, or this connection already
    joined. *)
...
(** Driver key state, sent whenever it changes. Pure intent — the server
    integrates it through physics, terrain and active powerdowns; the
    result appears in the next snapshot as this car's {!Velocity.t}.
    One-way because at input rates there is nothing useful to do with a
...
(** Driver uses a powerup they hold. Errors: not in inventory, not a
    driver, race not running, or (for {!Powerup.Flame_magic}) no ice
    nearby. *)
...
(** Track player invokes sabotage. Errors: not a track player, race not
    ...
    whatever {!Map_state.apply_action} rejects (unknown feature, wrong
    kind, not in its rest phase, ice off the road). *)
...
(** Track player grants a powerup from their stock to their own driver, who
    ...
    stock, not a track player, driver hasn't joined yet. *)
