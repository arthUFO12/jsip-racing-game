...
(* Expected shape: look the target feature up by id; check the action
   matches its kind and that it is in its rest phase ([Intact] / [Open] /
   [Hanging]); move it to its telegraph phase with an expiry of [now] +
   the relevant duration constant. [Place_ice] instead validates that
   [Cell.in_radius] around the target is all open road
   ([Map_state.surface_at] = [Road], no gap), allocates a fresh
   [Feature_id.t] (one past the max id in [t.features]), and inserts the
   patch. Every path returns the new [t] plus [Set]s for what changed;
   failures are [Or_error.error_s] with the id/action for context. *)
let apply_action
  (_ : t)
  ~map:(_ : Game_map.t)
...
(* Expected shape: fold over [t.features]; any phase whose expiry is
   [<= now] advances ([Collapsing -> Collapsed of { rebuilt_at }],
   ...
   [Removed]. Durations are tick-count constants defined at the top of
   this file until a config module exists. *)
let tick (_ : t) ~now:(_ : Tick.t) = failwith "TODO: implement Map_state.tick"
...
(* Expected shape for the composed queries: find the features whose
   [cells] contain the queried cell; apply [Feature.surface_override]
   (solid-most wins — authored features never overlap, validated at
   load, so ties only involve a spawned ice patch, which never
   overrides); [is_gap] and [traction_at] similarly consult
   [Feature.is_gap] / [Feature.traction_multiplier]. A [Cell.t ->
   Feature.t list] index built once per update would make these O(1);
   start with the naive scan and measure before bothering. *)
let surface_at (_ : t) ~map:(_ : Game_map.t) (_ : Vec2.t) =
...
let traction_at (_ : t) ~map:(_ : Game_map.t) (_ : Vec2.t) =
